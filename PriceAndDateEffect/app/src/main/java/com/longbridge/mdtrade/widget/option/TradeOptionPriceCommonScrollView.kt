 ...
 */
@Deprecated("不需要使用了")
open class TradeOptionPriceCommonScrollView @JvmOverloads constructor(
    context: Context,
    ...
    companion object{
         ...
         private const val TAG = "PriceCommonView_"
    ...
    private val flowLayout: OptionPriceLinearLayoutManager by lazy { OptionPriceLinearLayoutManager(context) }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        LogUtils.d(TAG, "onMeasure done measuredWidth=$measuredWidth measuredHeight=$measuredHeight")
    }
    ...
    private fun createCollectionView(): RecyclerView {
        ...
        return configureRecyclerViewCommon(
            ...
            adapter = this@TradeOptionPriceCommonScrollView.adapter,
    ...
    private fun createAdapter(): TradeOptionPriceAdapter {
        val adapter = TradeOptionPriceAdapter(
            ...
            object : OnExpirePriceClickListener {
                override fun onExpirePriceClick(position: Int, priceInfo: StrikePriceInfo) {
                    LogUtils.d(TAG, "onExpirePriceClick: ")
    ...
    fun scrollToNearestOption() {
        ...
        if (centerIndexPath >= 0) {
            ...
            LogUtils.d(TAG, "scrollToNearestOption centerIdx=$centerIndexPath delta=$delta")
            val smoothScroller = CenterSmoothScroller(context)
            smoothScroller.targetPosition = centerIndexPath
            if (abs(delta) > 0.5f) {
                pendingHapticAfterSnap = true
                LogUtils.d(TAG, "startSmoothScroll (snap) to $centerIndexPath")
                flowLayout.startSmoothScroll(smoothScroller)
            } else {
                pendingHapticAfterSnap = false
                LogUtils.d(TAG, "no snap needed")
            }
        }
    }
        ...
        if (inScrollOrLayout) {
            ...
            val runnable = Runnable {
                ...
                if (idx != null && idx != selectedIndex) {
                    LogUtils.d(TAG, "deferred updateSelectedState: $selectedIndex -> $idx (notify adapter)")
                    selectedIndex = idx
                    adapter.updateSelectedPosition(idx)
                    // 将震动交给 IDLE 时机统一处理，避免滚动中多次触发
                } else {
                    LogUtils.d(TAG, "deferred updateSelectedState: skip (idx=$idx, selected=$selectedIndex)")
                }
            }
            ...
            LogUtils.d(TAG, "updateSelectedState: $selectedIndex -> $centerIndexPath (notify adapter now)")
    ...
    private fun updateMaskSizeForCenterCell() {
        LogUtils.d(TAG, "updateMaskSizeForCenterCell()")
        val centerCell = findCenterCell() ?: return
        ...
        val screenCenterX = width / 2f
        LogUtils.d(TAG, "mask centerX(screenCenter)=$screenCenterX measuredWidth=$measuredWidth")
    ...
    public fun Float.dpToPx(): Int = (this * context.resources.displayMetrics.density).roundToInt()
...
private class OptionPriceLinearLayoutManager(context: Context) : LinearLayoutManager(context, HORIZONTAL, false) {
    companion object{
