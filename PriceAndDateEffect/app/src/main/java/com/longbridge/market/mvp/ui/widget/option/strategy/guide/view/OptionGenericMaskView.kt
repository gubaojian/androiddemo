 ...
 * 中间抠空的蒙层视图
 */
class OptionGenericMaskView @JvmOverloads constructor(
    context: Context,
    ...
    /** 抠空区域的大小（默认 80x80） */
    var clearRectSize: Size = Size(100.dpToPx(), 80.dpToPx())
        set(value) {
            val oldSize = field
            field = value
            Log.d(TAG, "clearRectSize set: ${oldSize.width}x${oldSize.height} -> ${value.width}x${value.height}")
    ...
    var clearRectCenterX: Float? = null
        set(value) {
            field = value
            Log.d(TAG, "clearRectCenterX set: $value (will updateMaskPath)")
    ...
    private fun updateMaskPath(size: Size) {
        Log.d(TAG, "updateMaskPath size=${size.width}x${size.height} centerX=$clearRectCenterX viewW=$width viewH=$height")
    ...
    private fun animatePathChange(oldSize: Size, newSize: Size) {
        Log.d(TAG, "animatePathChange from ${oldSize.width}x${oldSize.height} to ${newSize.width}x${newSize.height}")
        val animator = ValueAnimator.ofFloat(0f, 1f).apply {
            ...
            doOnEnd {
                ...
                Log.d(TAG, "animatePathChange end -> apply final size ${newSize.width}x${newSize.height}")
                updateMaskPath(currentAnimatedSize)
            }
        }
        animator.start()
