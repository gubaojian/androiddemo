...
fun ImageView.getIdDesc():String {
    if (id != ImageView.NO_ID) {
        return ""
    }
...
fun ImageView.setUrl(url: String?) {
    ...
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setUrl url ${url}")
    }
    Glide.with(context).load(url)
        .placeholder(mipmap.default_img) // 占位符，异常时显示的图片
        .error(mipmap.default_img) // 错误时显示的图片
        .skipMemoryCache(false) //启用内存缓存
        .diskCacheStrategy(DiskCacheStrategy.RESOURCE) //磁盘缓存策略
        .into(this)
}

fun ImageView.setUrlWithDrawablePlaceholder(url: String?) {
    if (ActivityManager.isActivityDestroy(context)) {
        return
    }
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setUrlWithDrawablePlaceholder url ${url}")
    }
    if (drawable != null) {
        Glide.with(context).load(url)
            .placeholder(drawable) // 占位符，异常时显示的图片
            .error(mipmap.default_img) // 错误时显示的图片
            .skipMemoryCache(false) //启用内存缓存
            .diskCacheStrategy(DiskCacheStrategy.RESOURCE) //磁盘缓存策略
            .into(this)
...
fun ImageView.loadFile(file: File?) {
    ...
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView loadFile url ${file}")
    }
    //请求配置
...
fun ImageView.setUrlNoCache(url: String?) {
    ...
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setUrlNoCache url ${url}")
    }
    Glide.with(context).load(url)
            .placeholder(mipmap.default_img)
            .error(mipmap.default_img)
...
fun ImageView.setUrlCircle(url: String?) {
    ...
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setUrlCircle url ${url}")
    }
    //请求配置
    val options = RequestOptions.circleCropTransform()
    Glide.with(context).load(url)
...
fun ImageView.setUrlRound(url: String?, radius: Int = 10) {
    ...
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setUrlRound url ${url}")
    }
    Glide.with(context).load(url)
            .placeholder(mipmap.default_img)
            .error(mipmap.default_img)
...
fun ImageView.setUrlAsBitmap(url: String?, callBack: (Bitmap) -> Unit) {
    if (ActivityManager.isActivityDestroy(context)) return
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setUrlAsBitmap url ${url}")
    }
...
fun ImageView.setUrlGif(url: String?) {
    if (ActivityManager.isActivityDestroy(context)) return
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setUrlGif url ${url}")
    }
...
fun ImageView.setBlurView(url: String?, radius: Int = 25, sampling: Int = 1) {
    ...
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setBlurView url ${url}")
    }
    //请求配置
...
fun ImageView.setScanImage(url: String?) {
    ...
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setScanImage url ${url}")
    }
            ...
            .into(object : CustomTarget<Drawable?>() {
                override fun onResourceReady(resource: Drawable, transition: Transition<in Drawable?>?) {
                    ...
                    windowManager.defaultDisplay.getMetrics(metrics)
                     lp.width =min(metrics.heightPixels, metrics.widthPixels)
...
fun ImageView.setHeaderImage(url: String?) {
    if (BuildConfig.DEBUG) {
        LogUtils.d("ImageView", "${getIdDesc()} ImageView setHeaderImage url ${url}")
    }
    if (!TextUtils.isEmpty(url)) {
       ...
       setImageResource(R.mipmap.ic_user_default)
    }
}
