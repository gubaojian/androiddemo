...
class CompanyAddMemberRepository(private val loadState: MutableLiveData<State>): BaseRepository() {
    ...
    suspend fun updateEntContract(parameters:HashMap<String,Any>): BaseResponse<Boolean> {
        return apiService.updateEntContract(parameters).requireLogin()
    }

    suspend fun addCompanyMember(parameters:HashMap<String,Any>): BaseResponse<Boolean> {
        return apiService.addCompanyMember(parameters).requireLogin()
    }

    suspend fun editCompanyMember(parameters:HashMap<String,Any>): BaseResponse<Boolean> {
        return apiService.editCompanyMember(parameters).requireLogin()
    }

    suspend fun deleteCompanyMember(userId: Long): BaseResponse<Boolean> {
        val parameter = hashMapOf<String, Any>();
        parameter.put("id", userId);
        return apiService.removeCompanyMember(parameter)
    }


    suspend fun getCompanyRoleList(): BaseResponse<List<RoleItem>>{
        return apiService.getCompanyRoleList().requireLogin()
    }
