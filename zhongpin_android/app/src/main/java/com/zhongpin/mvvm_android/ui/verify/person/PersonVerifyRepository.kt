...
class PersonVerifyRepository(private val loadState: MutableLiveData<State>): BaseRepository() {
    ...
    suspend fun identifyIdCardInfo(filePath:String): BaseResponse<IdCardInfoResponse> {
        ...
        return apiService.identifyIdCard(filePart)
    }
    ...
    suspend fun submitUserInfoAuth(parameters:HashMap<String,Any>): BaseResponse<Boolean> {
        return apiService.submitUserInfoAuth(parameters)
    }
