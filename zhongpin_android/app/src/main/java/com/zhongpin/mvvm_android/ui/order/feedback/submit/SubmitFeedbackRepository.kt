...
class SubmitFeedbackRepository(private val loadState: MutableLiveData<State>): BaseRepository() {
    ...
    suspend fun editOrderFeedback(parameters:HashMap<String,Any>): BaseResponse<Boolean> {
        return apiService.editOrderFeedback(parameters).requireLogin()
    }

    suspend fun addOrderFeedback(parameters:HashMap<String,Any>): BaseResponse<Boolean> {
        return apiService.addOrderFeedback(parameters).requireLogin()
    }
}