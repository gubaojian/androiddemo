...
import com.zhongpin.mvvm_android.common.utils.Constant.COMMON_KEY
...
abstract class BaseVMFragment<VM : BaseViewModel<*>> : BaseFragment(),BaseView {
    ...
    override fun setContentView(inflater: LayoutInflater, container: ViewGroup?): View {
        ...
        mViewModel = getFragmentViewModelProvider(this).get(CommonUtils.getClass(this))
        return rootView
    }
        ...
        Observer<State> {
            it?.let {
                ...
                when (it.code) {
                    ...
                    StateType.ERROR -> showError(errorMsg, it.urlKey)
                    StateType.NETWORK_ERROR -> showError(errorMsg, it.urlKey)
    ...
    fun showLoadingDialogV2() {
        ...
        if (mLoadingDialogV2 == null) {
            mLoadingDialogV2 = LoadingDialog(requireActivity(), false)
        }
        mLoadingDialogV2?.showDialogV2(requireActivity())
    }
