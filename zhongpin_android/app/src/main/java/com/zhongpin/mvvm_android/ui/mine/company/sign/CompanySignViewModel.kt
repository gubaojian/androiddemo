...
class CompanySignViewModel  : BaseViewModel<CompanySignRepository>() {
    ...
    fun signEntInfoAuth(parameters:HashMap<String,Any>) : MutableLiveData<BaseResponse<Boolean>> {
        val mLiveData: MutableLiveData<BaseResponse<Boolean>> = MutableLiveData()
        initiateRequest({
            mLiveData.value = mRepository.signEntInfoAuth(parameters);
        }, loadState)
        return mLiveData
    }
