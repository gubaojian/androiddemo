...
class SubmitFeedbackViewModel  : BaseViewModel<SubmitFeedbackRepository>() {
    ...
    fun editOrderFeedback(parameters:HashMap<String,Any>) : MutableLiveData<BaseResponse<Boolean>> {
        val mLiveData: MutableLiveData<BaseResponse<Boolean>> = MutableLiveData()
        initiateRequest({
            mLiveData.value = mRepository.editOrderFeedback(parameters);
        }, loadState)
        return mLiveData
    }

    fun addOrderFeedback(parameters:HashMap<String,Any>) : MutableLiveData<BaseResponse<Boolean>> {
        val mLiveData: MutableLiveData<BaseResponse<Boolean>> = MutableLiveData()
        initiateRequest({
            mLiveData.value = mRepository.addOrderFeedback(parameters);
        }, loadState)
        return mLiveData
    }
}