...
import com.zhilianshidai.pindan.app.databinding.ListFeedbackGridImageViewerItemBinding
import com.zhilianshidai.pindan.app.databinding.ViewFeedbackGridImageViewerBinding
import com.zhongpin.lib_base.utils.BannerUtils
import com.zhongpin.mvvm_android.ui.photo.preview.PhonePreviewerActivity

class FeedbackGridImageViewer @JvmOverloads constructor (context: Context,
                                                         attrs: AttributeSet? = null,
                                                         defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr)  {
    private lateinit var binding: ViewFeedbackGridImageViewerBinding;
    private val mData = mutableListOf<String>();
    private lateinit var mImageListAdapter:GridImageListAdapter;

    init {
        binding = ViewFeedbackGridImageViewerBinding.inflate(
    ...
    fun setImageUrls(imageUrls:List<String>) {
        ...
        if (imageUrls.size <= 3) {
            ...
            viewWith = SizeUtils.dp2px((65 + 10).toFloat()) * spanCount;
            viewHeight = SizeUtils.dp2px((65 + 10).toFloat())
        } else if (imageUrls.size <= 6) { //最多6张
            viewWith = SizeUtils.dp2px((65 + 10).toFloat()) * 3;
            viewHeight = SizeUtils.dp2px((65 + 10).toFloat()) * 2
        } else if (imageUrls.size <= 9) { //最多6张
            viewWith = SizeUtils.dp2px((65 + 10).toFloat()) * 3;
            viewHeight = SizeUtils.dp2px((65 + 10).toFloat()) * 4
        } else  { //最多6张
            viewWith = SizeUtils.dp2px((65 + 10).toFloat()) * 3;
            viewHeight = SizeUtils.dp2px((65 + 10).toFloat()) * 5
        }
        setViewWith(this@FeedbackGridImageViewer, viewWith, viewHeight);
    ...
    class GridImageListAdapter(val mContext: Context, val imageListData: List<String>)
        ...
        class VH(
            parent: ViewGroup,
            val binding: ListFeedbackGridImageViewerItemBinding = ListFeedbackGridImageViewerItemBinding.inflate(
