...
import com.zhongpin.lib_base.ktx.ViewClickDelay.SPACE_TIME
import com.zhongpin.lib_base.ktx.ViewClickDelay.hash
import com.zhongpin.lib_base.ktx.ViewClickDelay.lastClickTime
...
fun View.fadeIn(
    ...
    var animator: ValueAnimator? = null
    post {
        animator = ValueAnimator.ofFloat(alpha, 1.0f).apply {
            addUpdateListener {
...
fun View.fadeOut(
    ...
    var animator: ValueAnimator? = null
    post {
        animator = ValueAnimator.ofFloat(alpha, 0.0f).apply {
            addUpdateListener {
                alpha = it.animatedValue as Float
            }
            addListener(object : AnimatorListenerAdapter() {
...
/**
 * View点击事件扩展，添加500ms流控
 * @param interval 点击间隔时间(毫秒)，默认500ms
 * @param onClick 点击回调
 */
