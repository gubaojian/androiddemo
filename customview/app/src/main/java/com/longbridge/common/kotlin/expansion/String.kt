...
private fun getFormateListFromCounterId(counterId: String): Array<NumberFormat> {
    //TODO 未来适配其他市场
//    if (StockUtils.isStock(counterId)) {
//        return sUsFormateList
//    }
    return sUsFormateList
}

/**
 * 给0-8位小数位formater，应该足够涵盖所有场景
 * */
private val sUsFormateList = Array(20) { position ->
...
fun getUsFormateByContent(content: String, counterId: String): NumberFormat? {
    val decimalPart = content.split(".").getOrNull(1) ?: return getFormateListFromCounterId(counterId)[0]
    return getFormateListFromCounterId(counterId)[decimalPart.length]
}

private val numberRegex = Regex("[^\\d.\\-]")
fun String?.toPlaceHolderFormateUs(counterId: String? = ""): String {
    if (this.isNullOrEmpty()) return placeholder
    // 移除货币符号、逗号等非数字字符，保留负号、小数点和数字
    val cleanedString = this.replace(numberRegex, "")
    if (cleanedString.isEmpty()) return placeholder
    val prefix = if (cleanedString.startsWith("-")) "-" else ""
    val decimal = try {
        if (prefix.isEmpty()) cleanedString.toBigDecimal() else abs(cleanedString.toFloat()).toBigDecimal()
    } catch (e: Exception) {
        return placeholder
    }
    return "$prefix${getUsFormateByContent(cleanedString, counterId ?: "")?.format(decimal)}"
}
 ...
 * 判断String是否为数字或%
 */
fun String.isNumericOrPercent(): Boolean {
    return this.matches("-?\\d+(\\.\\d+)?%?$".toRegex())
}
