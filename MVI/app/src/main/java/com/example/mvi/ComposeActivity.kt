...
fun Greeting(name: String, modifier: Modifier = Modifier) {
    ...
    val uiState = produceState<DetailUiState>(initialValue = DetailUiState.Loading) {
        try {
            delay(1000)
            value = DetailUiState.Success("加载成功")
        }catch (e: Exception) {
            value = DetailUiState.Error("加载失败${e}")
        }
    }
    ...
    Column() {
        ...
        when (stateValue) {
            ...
            is DetailUiState.Success -> {
                Text("加载成功 ${stateValue.message}")
            }
            is DetailUiState.Error -> {
                Text("加载失败 ${stateValue.msg}")
            }
        }
        Button(onClick = {
