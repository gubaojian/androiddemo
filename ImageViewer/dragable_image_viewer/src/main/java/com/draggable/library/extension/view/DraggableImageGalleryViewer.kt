...
class DraggableImageGalleryViewer(context: Context) : FrameLayout(context) {
    ...
    private lateinit var mBinding: ViewImageViewrBinding

    init {
        LayoutInflater.from(context).inflate(R.layout.view_image_viewr, this)
        layoutParams = LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT)
        background = ColorDrawable(Color.TRANSPARENT)
        mBinding = ViewImageViewrBinding.bind(this)
        initAdapter()
        mBinding.mImageGalleryViewOriginDownloadImg.setOnClickListener {
            val currentImg = mImageList[mBinding.mImageViewerViewPage.currentItem]
    ...
    fun showImagesWithAnimator(imageList: List<DraggableImageInfo>, index: Int = 0) {
        ...
        mBinding.mImageViewerViewPage.adapter?.notifyDataSetChanged()
        setCurrentImgIndex(index)
    }

    private fun initAdapter() {
        mBinding.mImageViewerViewPage.adapter = object : androidx.viewpager.widget.PagerAdapter() {
            ...
            override fun instantiateItem(container: ViewGroup, position: Int): DraggableImageView {
                ...
                mBinding.mImageGalleryViewOriginDownloadImg.visibility = if (imageInfo.imageCanDown) View.VISIBLE else View.GONE
                return imageView
            }
        ...
        mBinding.mImageViewerViewPage.addOnPageChangeListener(object : androidx.viewpager.widget.ViewPager.OnPageChangeListener {
    ...
    private fun setCurrentImgIndex(index: Int) {
        mBinding.mImageViewerViewPage.setCurrentItem(index, false)
        mBinding.mImageViewerTvIndicator.text = "${index + 1}/${mImageList.size}"
    }
    ...
    fun closeWithAnimator(): Boolean {
        val currentView = findViewWithTag<DraggableImageView>("$TAG_PREGIX${ mBinding.mImageViewerViewPage.currentItem}")
        val imageInfo = mImageList[ mBinding.mImageViewerViewPage.currentItem]
